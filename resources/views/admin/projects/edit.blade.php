             ...
             <form action="{{ route('admin.projects.update', $project->slug) }}" method="POST" enctype="multipart/form-data"
                 ...
                 <div class="mb-3">
                     ...
                     <input type="text" class="form-control @error('git_link') is-invalid @enderror" id="git_link"
                         name="git_link" value="{{ old('git_link', $project->git_link) }}">
                     @error('git_link')
                         <div class="invalid-feedback">{{ $message }}</div>
                     @enderror
                 </div>
                 <div>
                     ...
                     <select name="diff_lvl" class="form-control @error('diff_lvl') is-invalid @enderror">
                         <option value="0" {{ old('diff_lvl', $project->diff_lvl == '0' ? 'selected' : '') }}>0
                         </option>
                         <option value="1" {{ old('diff_lvl', $project->diff_lvl == '1' ? 'selected' : '') }}>1
                         </option>
                         <option value="2" {{ old('diff_lvl', $project->diff_lvl == '2' ? 'selected' : '') }}>2
                         </option>
                         <option value="3" {{ old('diff_lvl', $project->diff_lvl == '3' ? 'selected' : '') }}>3
                         </option>
                         <option value="4" {{ old('diff_lvl', $project->diff_lvl == '4' ? 'selected' : '') }}>4
                         </option>
                         <option value="5" {{ old('diff_lvl', $project->diff_lvl == '5' ? 'selected' : '') }}>5
                         </option>
                         <option value="6" {{ old('diff_lvl', $project->diff_lvl == '6' ? 'selected' : '') }}>6
                         </option>
                         <option value="7" {{ old('diff_lvl', $project->diff_lvl == '7' ? 'selected' : '') }}>7
                         </option>
                         <option value="8" {{ old('diff_lvl', $project->diff_lvl == '8' ? 'selected' : '') }}>8
                         </option>
                         <option value="9" {{ old('diff_lvl', $project->diff_lvl == '9' ? 'selected' : '') }}>9
                         </option>
                         <option value="10" {{ old('diff_lvl', $project->diff_lvl == '10' ? 'selected' : '') }}>10
                         </option>
                     </select>
                     @error('diff_lvl')
                         <div class="invalid-feedback">{{ $message }}</div>
